...
public class SkeletonGenerator {
    ...
    public static SkeletonGenerator getInstance() throws Exception {
        return new SkeletonGenerator(getDefaultSystemValues());
    }
     ...
     * @return
     */
    public static SkeletonGenerator getInstance(Map<SystemValue, Object> values) throws Exception {
        return new SkeletonGenerator(values);
    }

    /* private constructor */
    private SkeletonGenerator(Map<SystemValue, Object> values) throws Exception {
        ...
        builder = initializeBuilder(values);
        termMap = new TermMap();
        Object languageAsObject = values.get(SystemValue.LANGUAGE);
        if (languageAsObject != null && languageAsObject instanceof CodePhrase) {
            language = (CodePhrase) languageAsObject;
        } else {
            //because who doesn't speak SV
            language = new CodePhrase("ISO_639-1", "en");
        }
    }

    private static Map<SystemValue, Object> getDefaultSystemValues() {
    ...
    private RMObjectBuilder initializeBuilder(Map<SystemValue, Object> systemValues) {
        return RMObjectBuilder.getInstance(systemValues);
    }
    ...
    String retrieveArchetypeTermText(String nodeId, Archetype archetype) throws Exception {
        String languageString = language.getCodeString();
        ArchetypeTerm term = archetype.getOntology().termDefinition(languageString, nodeId);
        if (term == null) {
            //fall back to English
            term = archetype.getOntology().termDefinition("en", nodeId);
            if (term == null) {
